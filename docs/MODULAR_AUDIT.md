...
Increment 2 (pending): port drops auto-mining into the same sidecar with full 1:1 settings parity per decision 14 (campaign selection, eligible-channel pick, synthetic watch, channel auto-switch, drop auto-claim, plus `priority_games`, `excluded_games`, `priority_mode`, `watch_interval_seconds`, `recovery_settings`, all surfaced in the plugin's host-rendered panel).
